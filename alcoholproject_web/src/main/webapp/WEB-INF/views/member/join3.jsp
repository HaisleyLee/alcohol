...
<h1 style="margin-bottom: 100px;">회원가입을 축하합니다!</h1>
		<form action="join4" method="post" enctype="multipart/form-data">
			<div><c:if test="${not empty vo.mem_profile_imgname}">
			<%-- ${vo.filename} --%>
		<img src="${vo.mem_profile_imgpath}" id="preview-img" class="file-img" alt="프로필사진" style="width: 100px; height: 100px; border-radius: 50px; object-fit: cover;"/>
		</c:if></div><br/>
			<div>${vo.mem_nickname}님</div><br/>
			<div>축하드립니다.</div><br/>
			<div>회원가입이 완료되었습니다.</div>
		</form>
		<div class="btnSet">
			<a class="btn-fill_join" onclick="go_join()">완료</a>	

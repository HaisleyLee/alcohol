 ...
 */
			...
			if(attach){
				...
				if($('#preview').length > 0 ){
					if(isImage(attach.name)){
						var img = '<img src="" class="file-img" id="preview-img" alt="프로필사진" style="width: 100px; height: 100px; border-radius: 50px; object-fit: cover;"/>';
						$('#preview').html(img);
		...
		$('#delete-file').on('click', function () {
			...
			//삭제시 이미지 미리보기 태그가 있는 경우 미리보기 이미지 없애기
			var img = '<img src="https://i.ibb.co/8r95NNC/1111111.png" style="width: 100px; height: 100px; border-radius: 50px;">';
			if($('#preview').length > 0) $('#preview').html(img);
		});
